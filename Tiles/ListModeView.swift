...
//  Tiles
//
//  Created by Yuki Sasaki on 2024/03/10.
//
...
struct ListModeView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
                ...
                NavigationView {
                    VStack{
                        SearchBar(text: $searchText, isSearching: $isSearching, sisRed: $sisRed, sisBlue: $sisBlue, sisYellow: $sisYellow, sisGreen: $sisGreen, sisPink: $sisPink, sisCheck: $sisCheck, sisNotCheck: $sisNotCheck, sisLiked: $sisLiked)
                            .disabled(isEditing)
                        ...
                        List {
                                ForEach(notDustArray, id: \.self) { item in
                                    ZStack {
                                        NavigationLink(destination: EditView(item: item), label: {
                                            ListModeCell(item: item)
                                                .frame(height: geometry.size.height * 1/12)
